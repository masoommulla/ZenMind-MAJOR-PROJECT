...
/* ── Each card manages its OWN video — only plays when in viewport ── */
function FeatureCard({ feature, index }: { feature: typeof features[0]; index: number }) {
  ...
  useEffect(() => {
    ...
    // Load only when card enters viewport; pause when scrolled away
  ...
  return (
    ...
    >
      ...
      >
        {/* video — preload="none" so browser doesn't fetch until play() is called */}
        <video
          ref={videoRef}
