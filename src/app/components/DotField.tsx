...
const DotField = memo(
  ({
    ...
    sparkle = false,
    waveAmplitude = 0,
    gradientFrom = 'rgba(13, 93, 58, 0.30)',
    gradientTo = 'rgba(110, 180, 120, 0.22)',
    glowColor = '#0d5d3a',
    showGlow = true,
    className = '',
    ...rest
      ...
      function tick() {
        ...
        for (let i = 0; i < dots.length; i += 1) {
          ...
          if (p.sparkle) {
            ...
            const currentRadius = (hash % 100) < 3 ? rad * 1.8 : rad;
            ctx.moveTo(drawX + currentRadius, drawY);
            ctx.arc(drawX, drawY, currentRadius, 0, TWO_PI);
          } else {
      ...
      <div className={`dot-field-container ${className}`.trim()} {...rest}>
        ...
        >
          ...
          <circle
            ...
            style={{ opacity: 0, willChange: 'opacity', display: showGlow ? 'block' : 'none' }}
          />
        </svg>
      </div>
    );
