...
// We need to replace the contents of the mobile nav.
// Look for `<nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">`
let start = content.indexOf('<nav className="flex-1 p-2 space-y-0.5 overflow-y-auto">');
...
// We need to replace the contents of the desktop nav.
// Look for `<nav className="flex-1 p-2 space-y-1">`
start = content.indexOf('<nav className="flex-1 p-2 space-y-1">');
end = content.indexOf('</nav>', start);
if (start !== -1 && end !== -1) {
  content = content.substring(0, start) + 
