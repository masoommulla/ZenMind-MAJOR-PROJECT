...
/* ══════════════════════════════════════════════════════════
   PUBLIC / USER ROUTES  — /api/wellness-programs/*
══════════════════════════════════════════════════════════ */

/** GET  /  — all published programs (with enrollment status for logged-in user) */
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    const programs = await WellnessProgram.find(q)
      .select('-steps') // steps loaded on detail view only
    ...
    const enriched = programs.map(p => ({
      ...p,
      enrollment: enrollMap[String(p._id)] || null,
    }));

    res.json({ ok: true, programs: enriched });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

/** GET  /:id  — full program detail + user enrollment */
...
/* ══════════════════════════════════════════════════════════
   ADMIN ROUTES  — /api/wellness-programs/admin/*
══════════════════════════════════════════════════════════ */

/** GET  /admin/list  — all programs */
